...
function parsePracticeQuestions(raw: string | null): any[] | null {
  if (!raw) return [];
  try {
...
function normalizeTopicConfig(value: string | null | undefined): string {
  return (value ?? "").trim();
}

function normalizeDifficultyConfig(value: string | null | undefined): string {
  return !value || value === "Mixed" ? "" : value;
}

function hasActiveSubscriptionStatus(user: { subscriptionStatus?: string | null } | null): boolean {
...
export async function POST(req: Request) {
  try {
    ...
    let existingPracticeTest: AppendPracticeTest | null = null;
    let initialExistingQuestionsForGeneration: any[] = [];
    ...
    if (existingTestId) {
      ...
      const parsedExistingQuestions = parsePracticeQuestions(existingPracticeTest.questions);
      if (!parsedExistingQuestions) {
        return NextResponse.json(
          { error: "Existing practice test questions are corrupted and cannot be appended." },
          { status: 409 }
        );
      }
      ...
      if (
        ...
        normalizeTopicConfig(existingPracticeTest.topic) !== normalizeTopicConfig(topic) ||
        normalizeDifficultyConfig(existingPracticeTest.difficulty) !==
          normalizeDifficultyConfig(difficulty)
      ) {
        return NextResponse.json(
          { error: "Append request must match the existing practice test configuration." },
          { status: 409 }
        );
      }

      if (parsedExistingQuestions.length + questionCount > MAX_PRACTICE_QUESTIONS) {
        return NextResponse.json(
          { error: `Practice tests cannot exceed ${MAX_PRACTICE_QUESTIONS} questions.` },
          { status: 400 }
        );
      }

      const devBypass =
        process.env.NODE_ENV !== "production" && process.env.BYPASS_PREMIUM_GATE === "true";
      if (
        existingPracticeTest.createdAt < getCurrentMonthStart() &&
        !devBypass &&
        !hasActiveSubscriptionStatus(accessContext.user)
      ) {
        return NextResponse.json(
      ...
      initialExistingQuestionsForGeneration = parsedExistingQuestions;
    } else {
...
- Reading/Writing: vary question types across official SAT domains and avoid identical phrasing.`;
    ...
    const existingQuestionsForGeneration = initialExistingQuestionsForGeneration;
    ...
    if (existingPracticeTest) {
      if (normalizedQuestions.length < questionCount) {
        return NextResponse.json(
          {
            error: "Could not generate a complete append batch. Please try again.",
            requested: questionCount,
      ...
      let appendPersisted = false;
      for (let attempt = 0; attempt < 5 && !appendPersisted; attempt += 1) {
        ...
        const existingQuestions = parsePracticeQuestions(latestPracticeTest.questions);
        ...
        if (
          latestPracticeTest.section !== section ||
          normalizeTopicConfig(latestPracticeTest.topic) !== normalizeTopicConfig(topic) ||
          normalizeDifficultyConfig(latestPracticeTest.difficulty) !==
            normalizeDifficultyConfig(difficulty)
        ) {
          return NextResponse.json(
            { error: "Append request must match the existing practice test configuration." },
        ...
        if (existingQuestions.length + normalizedQuestions.length > MAX_PRACTICE_QUESTIONS) {
          return NextResponse.json(
            { error: `Practice tests cannot exceed ${MAX_PRACTICE_QUESTIONS} questions.` },
            { status: 400 }
          );
        ...
        if (updateResult.count === 1) {
          ...
          practiceTestId = latestPracticeTest.id;
