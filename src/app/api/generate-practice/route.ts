  ...
  try {
    ...
    if (existingTestId) {
      existingPracticeTest = await prisma.practiceTest.findFirst({
        ...
        select: { id: true, questions: true, passage: true, createdAt: true, completedAt: true },
      });
      ...
      if (existingQuestionsForGeneration.length + questionCount > MAX_QUESTIONS_PER_PRACTICE_TEST) {
        return NextResponse.json(
          { error: `Practice tests cannot exceed ${MAX_QUESTIONS_PER_PRACTICE_TEST} questions.` },
          { status: 400 }
        );
