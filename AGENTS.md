...
**Product features (four tools):** Practice Tests, Study Plans, Micro-Lessons, and Progress. Flashcards were removed; Micro-Lessons cover short-form skill review.
...
- **OpenAI API key optional for basic navigation:** AI-powered features (practice test generation, study plans, micro-lessons) require `OPENAI_API_KEY`, but the app boots and non-AI pages work without it. The OpenAI client is lazily initialized (`src/lib/openai.ts`) so the build succeeds even without the key.
...
- **SQLite DB file:** `dev.db` is gitignored, so it must be recreated on every fresh VM (after the update script runs). Use `npx prisma migrate deploy` to create/apply the schema — it is non-interactive and exits cleanly. Avoid `npx prisma migrate dev` for first-time bootstrap here: it applies migrations but then hangs (does not return to the prompt) in this environment, and a killed run can leave a `dev.db-journal` lock (`database is locked`); if that happens, kill the lingering `prisma`/`schema-engine` processes and delete `dev.db-journal`.
